...
function playerspeed()
{
	...
	{
		if(human.IsAlive())
		human.RemoveInflictions()//移除感染和出血
		human.EnableSprint(false)//禁止奔跑
		EntFireByHandle(speed,"ModifySpeed","1.4",0,human,human);
		EntFire("rule","ApplyCvars","sv_gravity 550",0,null);
	}
}
...
function SetRandomColor() 
{
	...
	while ((ent = Entities.FindByName(ent, "rtv_beam_2*")) != null) {
		ent.__KeyValueFromVector("rendercolor", color)
	}
}
...
{
	local arc = RandomInt(0, 360)
	local vec = Vector(-5784 + 1276 * cos(arc * PI / 180), -10760 + 1276 * sin(arc * PI / 180), -11591)
